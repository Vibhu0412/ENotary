...
const Notification = () => {
  ...
  const isnotary = location.pathname.startsWith("/notary");
  const issignee = location.pathname.startsWith("/signee");
  return (
    <Dropdown classMenuItems="md:w-[300px] top-[58px]" label={notifyLabel()}>
      <div className="flex justify-between px-4 py-4 border-b border-slate-100 dark:border-slate-600">
        ...
        <div className="text-slate-800 dark:text-slate-200 text-xs md:text-right">
         { isadmin ? <Link to="/admin/notifications" className="underline">
            View all
          </Link>
: ''}
  { isorg ? <Link to="/organization/notifications" className="underline">
            View all
          </Link>
: ''}
  { isnotary ? <Link to="/notary/notifications" className="underline">
            View all
          </Link>
: ''}
  { issignee ? <Link to="/signee/notifications" className="underline">
            View all
          </Link>
: ''}
        </div>
